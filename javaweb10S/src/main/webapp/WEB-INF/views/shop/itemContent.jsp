...
<head>
    	...
    	$("#selectOption").change(function(){
    		...
    		if($("#layer"+idx).length == 0 && selectOption != "") {		// 옵션이 하나라도 있으면 처리하게 된다.
    		  idxArray[idx] = idx;
	    		...
	    		str += '<input type="hidden" id="price'+idx+'" value="'+optionPrice+'"/><span style="font-size:18px;">원</span>&nbsp;';			/* 변동되는 가격을 재계산하기위해 price+idx 아이디를 사용하고 있다. */
  ...
  <style>
    ...
    h2 {font-family:'EF_watermelonSalad';font-size:25px;}
    ...
    #h {border:2px solid #937062;width:80%;margin-left:auto;margin-right:auto;}
    #hh {width:100%;margin-left:auto;margin-right:auto;}
    #tt {font-family:'SUITE-Regular';font-size:15px;}
  </style>
</head>
<body>
...
<div class="container">
  <div class="row">
    <div class="col p-3 text-center" style="border:none;">
		...
		<div class="col p-3 text-left">
		  <!-- 상품 기본정보 -->
		  <div><h3 class="ml-4 mb-4 font-weight-bold">${productVO.productName}</h3></div>
		  <c:if test="${productVO.discountRate == 0}">
			  <div>
			    <h3 class="ml-4 mt-4 font-weight-bold"><fmt:formatNumber value="${productVO.price}"/>원</h3>
			  </div>
		  </c:if>
		  <c:if test="${productVO.discountRate != 0}">
			  <div>
			  	<del class="pp font-weight-bold" style="margin-left:25px;"><fmt:formatNumber value="${productVO.price}"/>원</del>
			  </div>
			  <div>
			  	<h2 class="ml-4 font-weight-bold"><font color="orange">${productVO.discountRate}%</font>&nbsp;&nbsp;<fmt:formatNumber value="${productVO.discount}" />원</h2>
			  </div>	
		  </c:if>
		  <hr />
		      ...
		      <select size="1" id="selectOption" class="text-center" style="width:200px;height:50px;">
		        ...
		        <c:forEach var="vo" items="${optionVOS}">
		          <option value="${vo.idx}:${vo.optionName}_${vo.optionPrice}">${vo.optionName}</option>
		       	...
		       	<!-- 장바구니보기/주문하기 처리 -->
		    		<button type="button" class="w3-button w3-ripple w3-brown mr-1 ml-1" onclick="cart()" style="width:155px;height:50px;">장바구니</button>
		    		<button type="button" class="w3-button w3-border w3-border-red w3-round mr-2 ml-2" onclick="location.href='${ctp}/shop/itemList'" style="width:155px;height:50px;">계속쇼핑하기</button>
		    </form>
		  </div>
		  <br />
		  <div>
		  ...
		  <div>
	        ...
	        <b><input type="text" class="totalPrice" id="totalPrice" value="<fmt:formatNumber value='0' />" readonly /><span style="font-size:20px;">원</span></b>
		    </div>
		  </div>
		  <br/>
		</div>
  ...
  <br />
  <div>
  	<h2 style="color:#760c0c;" class="text-center font-weight-bold">상품평</h2><hr id="hh" />
  		<table class="table table-borderless text-center tt" style="margin:auto;width:30%;"><!-- border:1px solid #000; -->
  		<c:forEach var="reviewVO" items="${reviewVOS}" varStatus="st">
				<tr>
					<td style="font-size:15px;color:#808080;">
			   		<!-- 짝수 별 -->
				    <c:if test="${reviewVO.star%2 == 0}">
				    	<c:forEach begin="0" end="4" step="1" varStatus="st" >
				      	<c:if test="${st.index+1 <= reviewVO.star/2}">
				    ...
				    <!-- 홀수 별 -->
				    <c:if test="${reviewVO.star%2 != 0}">
				    	<c:set var="str" value="${(reviewVO.star-1)/2}" />
				    	<c:forEach begin="0" end="4" step="1" varStatus="st" >
				  	...
				  	<br />
						${fn:substring(reviewVO.reviewDate,0,10)}&nbsp;&nbsp;&nbsp;${reviewVO.mid}<br /><span style="font-size:20px;" class="font-weight-bold">${reviewVO.content}</span></td>
					<td>
						<c:if test="${!empty reviewVO.photo}"><img src="${ctp}/review/${reviewVO.photo}" width="100px;" /></c:if>
					</td>
				</tr>  		
			</c:forEach>
  		</table>
